...
class _StringGenerator(object):
  ...
  def next(self, debug=False):
    ...
    if self.index < len(self.string):
      if debug:
        print "next[%d]='%s'"%(self.index, self.string[self.index])
...
class JsonReader(object):
    ...
    def _read(self):
        ...
        if peek == '{':
            ...
            raise ReadException, "Input is not valid JSON: '%s'" % peek

    def _readTrue(self):
        map(lambda x: self._assertNext(x, "true"),"true")
    ...
    def _readFalse(self):
        map(lambda x: self._assertNext(x, "true"),"false")
    ...
    def _readNull(self):
        map(lambda x: self._assertNext(x, "true"),"null")
    ...
    def _readNumber(self):
        ...
        result = self._next()
        peek = self._peek()
        while peek is not None and (peek.isdigit() or peek == "." or 
            peek.upper()=='E' or peek=='-' or peek=='+'):
    ...
    def _peek(self):
        ch=self._generator.peek()
        #~ sys.stdout.write(ch)
        return ch

    def _next(self, debug=False):
        ch=self._generator.next(debug=debug)
        #~ sys.stdout.write(ch)
        return ch
...
class JsonWriter(object):
    ...
    def _append_before(self, s):
        l=self._results[-1]
        self._results[-1]=s
        self._results.append(l)

    def write(self, obj, escaped_forward_slash=False):
        self._escaped_forward_slash = escaped_forward_slash
        self._results = []
        self.newline=False
        self._write(obj, self.indent)
        return "".join(self._results)

    def _write(self, obj, indent):
        mynewline=self.newline
        self.newline=False
        ty = type(obj)
        ...
        if ty is types.DictType:
            ...
            for k, v in obj.items():
                self._write(k,indent)
                self._append(":")
                self._write(v,indent)
                n = n - 1
                if n > 0:
                    #~ if mynewline:
                      #~ self._append_before(",")
          ...
          self._append(repr(ty)+" ** "+str(obj))
            #~ raise WriteException, "Cannot write in JSON: %s, type %s" % (repr(obj),repr(ty))
            

def write(obj, escaped_forward_slash=False):
    return JsonWriter().write(obj, escaped_forward_slash)

def write_debug(obj, escaped_forward_slash=False):
    return JsonWriter("\n").write(obj, escaped_forward_slash)
